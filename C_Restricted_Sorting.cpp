...
bool check(int k, int n, const vector<int>& a, const vector<int>& b) {
    int mn = a[0], mx = a[0];
    for (int x : a) {
        mn = min(mn, x);
        mx = max(mx, x);
    ...
    for (int i = 0; i < n; i++) {
        if (a[i] != b[i]) {
            // If this element needs to move, it must be able to swap
            // with either the min or the max to enter the "sorting pool"
...
void solve() {
    ...
    // The answer must be between 1 and the max possible difference
