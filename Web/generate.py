...
def detect_metadata(content, filename, folder_name):
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            index = match.group(3)
            # For problemset, adjust contest_id to full ID
            if 'problemset/problem' in pattern:
                contest_id = f"{contest_id}{index}"
            break
    ...
    for folder in DIRECTORIES:
        ...
        for root, _, files in os.walk(folder):
            for file in files:
                ...
                # Full rel path for duplicate check
                ...
                # Correct rel_path with subfolders
                rel_path = os.path.relpath(new_path, start=os.path.join(os.getcwd(), folder)).replace(os.sep, '/')
...
<p align="center">
    <a href="https://{os.getenv('GITHUB_REPOSITORY_OWNER', 'mhdnazrul')}.github.io/{os.getenv('GITHUB_REPOSITORY', 'Codeforces-Solutions').split('/')[-1]}/">
...
| :--- | :--- |
"""
    for t in sorted(stats['by_tag'].keys()):
        md += f"| {t} | {stats['by_tag'][t]} |\n"
...
"""
    ...
    for p in sorted_probs:
        ...
        row = f"| {p['id']} | {p['name']} | {rating_display} | {tags_display} | [View]({p['q_link']}) | [Code]({sol_full_link}) |\n"
        md += row
